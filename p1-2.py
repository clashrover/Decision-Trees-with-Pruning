...
def main(train,test,val,out_dir):
    ...
    y_predict = dTree.infer(dTree.getRoot(),(x,y))
    y_predict = y_predict[y_predict[:,0].argsort()]
    y_predict = y_predict[:,1:2]
    acc = 100*accuracy(y_predict,y)

    y_predict1 = dTree.infer(dTree.getRoot(),(x1,y1))
    y_predict1 = y_predict1[y_predict1[:,0].argsort()]
    y_predict1 = y_predict1[:,1:2]
    acc1 = 100*accuracy(y_predict1,y1)

    y_predict2 = dTree.infer(dTree.getRoot(),(x2,y2))
    y_predict2 = y_predict2[y_predict2[:,0].argsort()]
    y_predict2 = y_predict2[:,1:2]
    acc2 = 100*accuracy(y_predict2,y2)

    
    # search and prune the tree
    graphData =[(dTree.currentSize,acc,acc1,acc2)]
    ...
    print("after pruning test acc:",100*accuracy(y_predict,y1))
